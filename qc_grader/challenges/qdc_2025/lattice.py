...
class HeavyHexLattice:
    ...
    def edges_connected_to_node(self, node_coords):
        ...
        for coords in self.coords:
            if (0 <= (dx := abs(node_coords[0] - coords[0])) < 1) and (
                0 <= (dy := abs(node_coords[1] - coords[1])) < 1
            ):
    ...
    def nodes_connected_to_edge(self, edge_coords):
        ...
        return (
            (
                np.floor(edge_coords[0]).astype(int),
                np.floor(edge_coords[1]).astype(int),
            ),
    ...
    def plot_lattice(
        ...
        vertex_x = np.array(
            [v[1] for v in sorted(list(self.vertices.keys()))], dtype=int
        )
        vertex_y = np.array(
            [self.max_y - v[0] for v in sorted(list(self.vertices.keys()))], dtype=int
        )

        # Collect edge endpoints (line segments)
        edges_endpoints_x = np.array(
            [[np.floor(e[1]), np.ceil(e[1])] for e in sorted(list(self.edges.keys()))],
            dtype=int,
        )
        edges_endpoints_y = np.array(
            [
                [self.max_y - np.floor(e[0]), self.max_y - np.ceil(e[0])]
                for e in sorted(list(self.edges.keys()))
            ],
            dtype=int,
        )
    ...
    def initial_qubit_layout(self, first_qubit=None, backend=None, reflex=None):
        ...
        if (ibm_origin[0] + 1, ibm_origin[1]) in ibm_qubit_coords:
            ...
            for c in self.coords:
                offset = (
                    int(2 * (c[0] - own_origin[0])),
                    int(2 * (c[1] - own_origin[1])),
                )
                if reflex:
    ...
    def plot_highlighted_qubits(
        self,
        qubits,
        scale=1.5,
        number_qubits=False,
        number_highlighted=True,
        colors=None,
        filepath="",
    ):
        """
        ...
        non_highlighted_qubit_inds = list(
            set(range(len(self.coords))) - set(np.array(qubits).tolist())
        )

        # Map logical qubit indices to arrays-of-indices for plotting
        highlighted_nodes_inds = [
            np.where(np.equal(self.node_qbinds, qbind))[0][0]
            for qbind in qubits
            if not is_edge_coords(self.coords[qbind])
            and len(np.where(np.equal(self.node_qbinds, qbind))) > 0
        ]
        highlighted_edges_inds = [
            np.where(np.equal(self.edge_qbinds, qbind))[0][0]
            for qbind in qubits
            if is_edge_coords(self.coords[qbind])
            and len(np.where(np.equal(self.edge_qbinds, qbind))) > 0
        ]
        non_highlighted_nodes_inds = [
            np.where(np.equal(self.node_qbinds, qbind))[0][0]
            for qbind in non_highlighted_qubit_inds
            if not is_edge_coords(self.coords[qbind])
            and len(np.where(np.equal(self.node_qbinds, qbind))) > 0
        ]
        non_highlighted_edges_inds = [
            np.where(np.equal(self.edge_qbinds, qbind))[0][0]
            for qbind in non_highlighted_qubit_inds
            if is_edge_coords(self.coords[qbind])
            and len(np.where(np.equal(self.edge_qbinds, qbind))) > 0
        ...
        highlighted_nodes_color_inds = [
            i
            for i, qbind in enumerate(qubits)
            if not is_edge_coords(self.coords[qbind])
        ]
        highlighted_edges_color_inds = [
            i for i, qbind in enumerate(qubits) if is_edge_coords(self.coords[qbind])
        ]
        ...
        plt.scatter(
            vertex_x[highlighted_nodes_inds],
            vertex_y[highlighted_nodes_inds],
            350 * scale,
            marker="o",
            c=colors[highlighted_nodes_color_inds],
            edgecolors="black",
            zorder=2,
        )
        plt.scatter(
            vertex_x[non_highlighted_nodes_inds],
            vertex_y[non_highlighted_nodes_inds],
            350 * scale,
            marker="o",
            c="white",
            edgecolors="black",
