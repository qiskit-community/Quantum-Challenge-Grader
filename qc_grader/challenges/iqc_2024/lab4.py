...
def create_test_backend():
    ...
    for i in range(backend.num_qubits):
        ...
        backend.target.update_instruction_properties(
            "rz", qarg, InstructionProperties(error=error_rate, duration=1e-8)
        )
        backend.target.update_instruction_properties(
            "x", qarg, InstructionProperties(error=error_rate, duration=1e-8)
        )
        backend.target.update_instruction_properties(
            "sx", qarg, InstructionProperties(error=error_rate, duration=1e-8)
        )
        backend.target.update_instruction_properties(
            "measure", qarg, InstructionProperties(error=error_rate, duration=1e-8)
        )

    for edge in backend.coupling_map:
        backend.target.update_instruction_properties(
            "cx", tuple(edge), InstructionProperties(error=error_rate, duration=1e-8)
        )
...
@typechecked
def grade_lab4_ex1(
    list_coefficients: List[List[complex]], list_labels: List[int]
) -> None:
    grade(
        {
            "list_coefficients": list_coefficients,
            "list_labels": list_labels,
            "eq": list_coefficients == CORRECT_LIST_COEFFICIENTS
            and list_labels == CORRECT_LIST_LABELS,
        },
        "lab4-ex1",
        _challenge_id,
    )
...
@typechecked
def grade_lab4_ex2(num_qubits: int, reps: int, entanglement: str) -> None:
    answer = {"num_qubits": num_qubits, "reps": reps, "entanglement": entanglement}
...
def grade_lab4_ex3(optimize_result: OptimizeResult) -> None:
    ...
    pm = generate_preset_pass_manager(
        backend=AerSimulator(), optimization_level=1, seed_transpiler=0
    )
...
def grade_lab4_ex4(backend: BackendV2) -> None:
    grade(list(backend.coupling_map), "lab4-ex4", _challenge_id)


@typechecked
def grade_lab4_ex5(backend: BackendV2) -> None:
    basis_gates = ["rz", "x", "sx", "cx", "measure"]
    gate_error_list = [
        next(iter(backend.target[gate].values())).error for gate in basis_gates
    ]
    grade(gate_error_list, "lab4-ex5", _challenge_id)


@typechecked
def grade_lab4_ex6(zne_strategy: ZNEStrategy, optimize_result: OptimizeResult) -> None:
    ...
    for amplitudes, label in zip(list_coefficients, list_labels):
        ...
        job_zne = estimator.run(
            transpiled_classifier, transpiled_obs, opt_params, zne_strategy=zne_strategy
        )
