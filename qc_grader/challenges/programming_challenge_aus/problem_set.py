...
def grade_prob1_ex3(qc: QuantumCircuit, counts: Dict[str, int]) -> None:
    answer = {"qc": circuit_to_json(qc), "counts": counts}
...
def grade_prob2_ex1(tsp_function: Callable) -> None:
    ...
    for i in inputs.keys():
        distance, order = tsp_function(int(i), inputs[i])
        answer[i] = {"distance": distance, "order": order}
...
def grade_prob2_ex2(result: SamplingVQEResult) -> None:
    ...
    answer = {"solution_order": solution_order, "solution_distance": solution_distance}
